...
import PartnerKycSettingsModal, {
  PartnerKycSettingsModalData,
  formSchema as eventFormSchema,
} from "./PartnerKycSettingsModal";
...
interface PartnerKycSettingsProps {
  onSubmitSuccess?: () => void;
  isLoading?: boolean;
}

const PartnerKycSettings: React.FC<PartnerKycSettingsProps> = ({
  onSubmitSuccess,
  isLoading = false,
}) => {
  ...
  const handleAddOrEditEvent = useCallback(
    (event: PartnerKycSettingsModalData) => {
      try {
  ...
  return (
    <Container maxWidth="full">
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
        <h1 className="text-base font-semibold">KYC Form for Partner</h1>
        <PrimaryBtn
          variant="primary"
          onClick={() => {
            setEditingIndex(null);
...
export default PartnerKycSettings;
