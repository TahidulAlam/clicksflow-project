...
import ASUCustomInfoEditModal, {
  ASUCustomInfoEditModalData,
  formSchema as eventFormSchema,
} from "./ASUCustomInfoEditModal";
...
interface ASUCustomInfoEditProps {
  onSubmitSuccess?: () => void;
  isLoading?: boolean;
}

const ASUCustomInfoEdit: React.FC<ASUCustomInfoEditProps> = ({
  onSubmitSuccess,
  isLoading = false,
}) => {
  ...
  const handleAddOrEditEvent = useCallback(
    (event: ASUCustomInfoEditModalData) => {
      try {
  ...
  return (
    <Container maxWidth="full">
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 bg-blue-950 p-4 rounded-t-md">
        <h1 className="text-base text-white ">Custom Fields</h1>
        <PrimaryBtn
          variant="primary"
          onClick={() => {
            setEditingIndex(null);
...
export default ASUCustomInfoEdit;
