...
import PSUCustomInfoEditModal, {
  PSUCustomInfoEditModalData,
  formSchema as eventFormSchema,
} from "./PSUCustomInfoEditModal";
...
interface PSUCustomInfoEditProps {
  onSubmitSuccess?: () => void;
  isLoading?: boolean;
}

const PSUCustomInfoEdit: React.FC<PSUCustomInfoEditProps> = ({
  onSubmitSuccess,
  isLoading = false,
}) => {
  ...
  const handleAddOrEditEvent = useCallback(
    (event: PSUCustomInfoEditModalData) => {
      try {
  ...
  return (
    <Container maxWidth="full">
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 bg-blue-950 p-4 rounded-t-md">
        <h1 className="text-base text-white ">KYC Form for Partner</h1>
        <PrimaryBtn
          variant="primary"
          onClick={() => {
            setEditingIndex(null);
...
export default PSUCustomInfoEdit;
